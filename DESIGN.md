...
1. Coin is clickable if user hovers mouse over the name.
...
4. User's portfolio info: coins, amount, current value of coin and total value of coin holding. Coin name is clickable; displays coin page.
...
17. Creates a link to download the user's trade history in a CSV format.
...
21. Pop-up screen when user clicks 'add trade': (green: buy / red: sell). Current price and coin name is pre-filled but editable by user. Adds coin automatically to portfolio if not in portfolio already.
...
23. Field to write a note about the coin. 'Delete Note' button gets displayed when user wrote a note.
24. Confirmation pop-up shows up if user click 'Delete Note' button.
